...
parser.add_argument("--text_input", type=str, default="./example/human_300.txt", help="Input text or file path")
parser.add_argument("--ai_model", type=str, default="qwen2.5:7b", help="Ollama model name")
...
def process_similarity(segment_data):
    ...
    for i in range(len(segment_data)):
        ...
        if prev_tokens and curr_tokens and prev_emb.size > 0 and curr_emb.size > 0:
            bs_val = round(compute_bertscore(prev_tokens, prev_emb, curr_tokens, curr_emb), 4)
            processed_entries[i]["bertscore"] = float(bs_val)
        else:
            processed_entries[i]["bertscore"] = 0.0
